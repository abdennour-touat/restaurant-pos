                                ...
                                <td style='display:flex;'>${
                                  trans.paid == ""
                                    ...
                                    : '<button onClick="$(this).viewTransaction(' +
                                      index +
                                      ')" class="btn btn-info"><i class="fa fa-search-plus"></i></button><button style="margin-left: 5px;" class="btn btn-danger" onClick="$(this).deleteTransaction(' +
                                      index +
                                      ')">X</button></td>'
                                }
...
function customerFilter(customers) {
  $("#customers").empty();
  $("#customers").append(`<option value='0'> All </option>`);
  customers.forEach((cutmr) => {
    $("#customers").append(
      `<option value = ${cutmr.name}>${cutmr.name}</option>`
    );
  });
}
...
$("#customers").change(() => {
  by_customer = $("#customers").find("option:selected").val();
  // loadTransactions();
});
...
//the reset function to reset all the data...
function reset() {
  Swal.fire({
    title: "Are you sure?",
    text: "You are about to delete this product.",
